...
static  CPLPAGEINFO     grgcpInfo[NUMPAGES] = {
	...
	{ MAKEINTRESOURCEW(IDD_X36PROP_PAGE2), X36PROP_Page2_DlgProc}
};
...
//*************************************************
		...
		if(i==0) {
			char tit[1024];
			strcpy(tit,Traduce::Txt(LG_CPL_D1_GUI0));
			MultiByteToWideChar(CP_ACP,0,tit,strlen(tit),tit1,64);
			m_pdigcPageInfo[i].lpwszPageTitle    = tit1;
		} else {
			char tit[1024];
			strcpy(tit,Traduce::Txt(LG_CPL_D2_GUI0));
			MultiByteToWideChar(CP_ACP,0,tit,strlen(tit),tit2,64);
			m_pdigcPageInfo[i].lpwszPageTitle    = tit2;
		}
