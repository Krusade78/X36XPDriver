    ...
    )
{
	...
	for(axisIdx=0;axisIdx<MAX_AXES;axisIdx++) {
		if(DeviceExtension->PrevGoodAxis[axisIdx]==ejes[axisIdx]) {
			DeviceExtension->jitter[axisIdx].PosRepetida++;
		} else {
			DeviceExtension->jitter[axisIdx].PosRepetida=0;
		}
