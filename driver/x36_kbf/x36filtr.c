	...
	)
{
	...
	devExt->IsrWritePort(devExt->CallContext,0);
	// Acelerador y Timón
	devExt->IsrWritePort(devExt->CallContext,x36DevExt->acelPed);
	// Fin
...
--*/
{
	...
	switch(irpStack->MajorFunction) {
		...
		{
			if(irpStack->Parameters.DeviceIoControl.IoControlCode==IOCTL_HID_LEERX36)
			{
				if(irpStack->Parameters.DeviceIoControl.OutputBufferLength==sizeof(HIDGAME_INPUT_DATA)) {
					RtlCopyMemory(Irp->UserBuffer,&x36DevExt->hidData,sizeof(HIDGAME_INPUT_DATA));
