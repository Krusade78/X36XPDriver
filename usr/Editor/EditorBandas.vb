Public Class EditorBandas
    ...
    Public Sub New(ByVal p As Principal)
        ...
        padre = p
        padre.GetModos(modop, modom, modoa)
    ...
    Private Sub Traducir()
        Me.Text = Traduce.Txt(110)
        Label1.Text = Traduce.Txt(111)
        ButtonG.Text = Traduce.Txt(30)
        ButtonC.Text = Traduce.Txt(112)
    End Sub
    ...
    Private Sub Guardar()
        ...
        Dim nbandas As Byte = 0
        For i As Byte = 0 To 14
            If num(i).Value > 0 Then nbandas = nbandas + 1
        Next
        If nbandas >= poses Then
            Dim m1 As String = Traduce.Txt(84) & "<" & poses.ToString() & ")"
        ...
        For i As Byte = n To 14
            If num(i).Value = 0 Then
                If i = 0 Then
                    split(i).Height = 512
                Else
                    split(i).Height = 512 - (2 * num(i - 1).Value) '- i - 1
                End If
                ...
                Next
                Exit For
            ElseIf num(i).Value = 254 Then
                If i = 0 Then
                    split(i).Height = 2 * num(i).Value
                Else
                    split(i).Height = 2 * (num(i).Value - num(i - 1).Value)
                End If
                For j As Byte = i + 1 To 14
                    num(j).Value = 0
                    num(j).Enabled = False
            ...
            Else
                ...
                If i > 0 Then
                    If num(i).Value <= num(i - 1).Value Then
                        ...
                        If num(i).Value = 254 And i < 14 Then
                            num(i + 1).Value = 0
