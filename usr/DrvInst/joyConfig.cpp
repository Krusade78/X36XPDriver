...
bool CjoyConfig::Instalar(DWORD s_int,DWORD s_joy)
{
	...
	SetupDiDestroyDeviceInfoList(di);
	if(deviddata==NULL) {
		Traduce::Msg(NULL,LG_INTERFAZ,"CjoyConfig::Instalar[3]",MB_ICONSTOP);
		goto mal;
	}
	...
	if(!DeviceIoControl(driver,IOCTL_GAMEENUM_EXPOSE_HARDWARE,geh,sizeof(GAMEENUM_EXPOSE_HARDWARE)+hidlen,geh,sizeof(GAMEENUM_EXPOSE_HARDWARE)+hidlen,&i,NULL)) {
		Traduce::Msg(NULL,LG_ENUSO,"CjoyConfig::Instalar[6]",MB_ICONSTOP);
		CloseHandle(driver);
		goto mal1;
	}
...
bool CjoyConfig::EnumerarJoysticks(HWND hWnd)
{
	SendMessage(hWnd,LB_ADDSTRING,0,(LPARAM)"Saitek X36-F");
	SendMessage(hWnd,LB_ADDSTRING,0,(LPARAM)"Saitek X36-F + X35-T");

	return true;
}

bool CjoyConfig::EnumerarGameports(HWND hWnd,DWORD* idx)
{
	CONST GUID enumclass={0xcae56030,0x684a,0x11d0,{0xd6,0xf6,0x00,0xa0,0xc9,0x0f,0x57,0xda}};
	HDEVINFO di=SetupDiGetClassDevs(&enumclass,NULL,NULL,DIGCF_PRESENT|DIGCF_DEVICEINTERFACE );
	if(di==INVALID_HANDLE_VALUE)
	{
		Error("CjoyConfig::EnumerarGameports[0]");
		return false;
	}
	SP_DEVINFO_DATA dev;
	dev.cbSize=sizeof(SP_DEVINFO_DATA);
	...
	while(SetupDiEnumDeviceInfo(di,*idx,&dev)) {
		...
		{
			Error("CjoyConfig::EnumerarGameports[1]");
